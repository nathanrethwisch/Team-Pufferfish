...
According to the Iowa Department of Transportation, there are over 50,000 crashes per year. These crashes cause millions in property damage, and unfortunately, the loss of life, with over 300 people dying to vehicular crashes per year. It is important that we understand more about
...
#This is strange because it appears that Weather Conditions has NA values, but they are not appearign
...
df%>%
  group_by(sunriseDiff, `Travel Direction`)%>%
   at different crash severities with sunset vs. crash time
df%>%
...
dfDaylight%>%
  ggplot(aes(x = Daylight, y = total)) + geom_line() + ylab("Average Number of Crashes")
```

```{r}
dfDaylight%>%
  ggplot(aes(x = Daylight, y = total)) + geom_line() + ylab("Average Number of Crashes") + geom_smooth(span = 0.3)
...
<https://iowadot.gov/mvd/stats/crashhistory.pdf>
