...
One point of note is looking into how the length of the day affects car crashes, regardless of whether or not drugs or alcohol are present. \@ref(fig:daylight)shows the number of crashes based on the length of the day. A line of best fit was added to the graph using LOESS (Locally Weighted Scatterplot Smoothing).
...
We found that although crashes while under the influence are less common, they cause almost \$3000 more damage, on average. \@ref(fig:property)shows the difference in property damage between drunk and sober crashes.
...
Next we looked at when crashes happened among drivers. Similar to our analysis in section II, we found that there were spikes in crashes at sunrise and sunset hours. However, as shown in \@ref(fig:crashHour)crashes are inverted among drunk drivers, with most crashes happening between 11pm and 3 am.
...
Next, we decided to look at drinking by day of the week. We expected to see a large spike in drunk crashes on the weekends, especially Saturday. \@ref(fig:crashDay) shows a comparison between crashes among all drivers and those under the influence of drugs.
