...
```{r}
#Shows animal-related crashes 
crashes <- crashes %>%
 mutate(Hour = parse_number(Hour))

crashes %>%filter(`Major Cause` == "Animal")%>%
  ggplot(aes(x = Hour)) + geom_bar() + ylab("Number of Animal-Related Crashes") + xlab("Hour")
```
...
```{r, echo = FALSE, warning = FALSE, message = FALSE, fig.cap = "A graph of all crashes based on the length of the day with a line fitted using LOESS smoothing"}
...
crash %>% group_by(Year, Month, Day) %>% count() %>% 
