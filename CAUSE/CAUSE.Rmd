...
```{r, echo = FALSE, warning = FALSE, message = FALSE}
dat <- paste0("$", c(true_total_cost, false_total_cost))
...
gg1<- crash %>%
  ggplot(aes(x=Hour)) + geom_bar() + ggtitle("Crashes among all drivers")
# Most crashes happen during "rush hour" from 4 to 6 P.M.

# Plot of all crashes with sustance abuse by the hour
gg2<- crash %>%
  filter(Drug_Usage == TRUE) %>%
  ggplot(aes(x=Hour)) + geom_bar()+ ggtitle("Crashes among drunk drivers")
# Most crashes with substances involved happen from 11 P.M. to 3 A.M.
```

## Time of Day

Next we looked at when crashes happened among drivers. Similar to our analysis in section II, we found that there were spikes in crashes at sunrise and sunset hours. However, as shown in **INSERT FIGURE THING,** crashes are inversed among drunk drivers, with most crashes happening between
